...
const getViewTickets = (filters, method, ticketsArray) => {
  ...
  const filteredTickets = ticketsArray.filter((ticket) => {
    ...
    if(ALL_TRANSFERS) { return true }

    if(WITHOUT_TRANSFERS &&
      (thereStops === 0 || backStops === 0)) { return true }

    if(ONE_TRANSFERS &&
      (thereStops === 1 || backStops === 1)) { return true }

    if(TWO_TRANSFERS &&
      (thereStops === 2 || backStops === 2)) { return true }

    return THREE_TRANSFERS &&
      (thereStops === 3 || backStops === 3);
  });
  ...
  const compare = (aTicket, bTicket) => {
    ...
    const aDuration =
      aTicket.flightThere.duration +
      aTicket.flightBack.duration;

    /* Общая продолжительность рейсов второго билета */
    const bDuration =
      bTicket.flightThere.duration +
      bTicket.flightBack.duration;

    return  method === 'CHEAPEST' ?
      aTicket.price - bTicket.price :
      aDuration - bDuration
  };
